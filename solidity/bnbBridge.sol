...
contract bnbBridge is Multiownable {
    ...
    function sendTokens(uint256 amount) public {
        ...
        require(ERC20(tokenAddress).balanceOf(msg.sender) >= amount,"Not enough balance");
        
        transferStatus = ERC20(tokenAddress).transferFrom(msg.sender, address(this), amount);
    ...
    function recieveTokens(uint256[] memory commissions) public payable {
        if (tokensRecievedButNotSent[msg.sender] != 0) {
            ...
            transferStatus = ERC20(tokenAddress).transfer(msg.sender, amountToSent);
            if (transferStatus == true) {
    ...
    function withdrawTokens(uint256 amount, address reciever) public onlyAllOwners {
        ...
        require(ERC20(tokenAddress).balanceOf(address(this)) >= amount,"Not enough balance");
        
        ERC20(tokenAddress).transfer(reciever, amount);
    }
